...
def calculate_angle(a, b, c):
    ...
    Digunakan untuk menghitung sudut bahu dan sudut leher
    """
    a = np.array(a)
    b = np.array(b)
    c = np.array(c)
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        # Extract key body landmarks / Mengambil koordinat bahu kiri, bahu kanan dan telinga kiri
        ...
        shoulder_angle = calculate_angle(left_shoulder, right_shoulder, (right_shoulder[0], 0)) #Sudut antara bahu kiri, bahu kanan, dan titik diatas bahu kanan
        neck_angle = calculate_angle(left_ear, left_shoulder, (left_shoulder[0], 0)) #Sudut antara telinga kiri, bahu kiri dan titik diatas bahu kiri
        ...
        if not is_calibrated and calibration_frames < 30: #Sistem mengumpulkan 30 frame pertama untuk menentukan sudut rata-rata postur normal
            ...
            #shoulder_threshold dan neck_threshold ditentukan berdasarkan nilai rata-rata sudut dikurangi 10 derajat agar tdk terlalu sensitif
            #Default = 10
            is_calibrated = True
        ...
        # x.draw_landmarks(image/source, landmark_list, connection:OPTIONAL,landmark_drawing__spec : OPTIONAL, connection_drawing_spec,is_drawing_landmarks: bool = True,)
        ...
        # STEP 4: Feedback
        #Jika sudut lebih kecil dari threshold, postur dianggap buruk
        #Jika lebih dari cooldown (5 detik(alert cooldown)), peringatan diberikan.
        if is_calibrated:
